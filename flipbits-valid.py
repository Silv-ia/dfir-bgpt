...
def corrupt_chunk(chunk_path: Path, percent: float):
    """Run bitflip spray on a single binary chunk, keeping the same filename."""
    tmp_path = chunk_path.with_suffix(".tmp")
    cmd = [BITFLIP_CMD, "spray", f"percent:{percent}", str(chunk_path), str(tmp_path)]
    ...
    # Replace original file with corrupted version
